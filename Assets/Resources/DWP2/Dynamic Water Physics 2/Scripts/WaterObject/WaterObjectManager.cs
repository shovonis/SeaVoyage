...
namespace DWP2
{
    ...
    public class WaterObjectManager : MonoBehaviour
    {
        ...
        private void Initialize()
        {
            ...
            FastCopy(p0s, _waterTriJob.P0S);
            FastCopy(p1s, _waterTriJob.P1S);
            FastCopy(p2s, _waterTriJob.P2S);
            FastCopy(localToWorldMatrices, _waterTriJob.LocalToWorldMatrices);
        ...
        private void InitializeWaterDataProvider()
        {
        ...
        private void Schedule()
        {
            ...
            FastCopy(_waterTriJob.States, _states);
            int waterObjectCount = _waterObjects.Count;
            for (int i = 0; i < waterObjectCount; i++)
            {
            ...
            FastCopy(waterHeights0, _waterTriJob.WaterHeights0);
            FastCopy(waterHeights1, _waterTriJob.WaterHeights1);
            FastCopy(waterHeights2, _waterTriJob.WaterHeights2);
            ...
            FastCopy(waterVelocities0, _waterTriJob.WaterVelocities0);
            FastCopy(waterVelocities1, _waterTriJob.WaterVelocities1);
            FastCopy(waterVelocities2, _waterTriJob.WaterVelocities2);
            
            // Copy new data to native containers
            FastCopy(_states, _waterTriJob.States);
            FastCopy(_velocities, _waterTriJob.Velocities);
            FastCopy(localToWorldMatrices, _waterTriJob.LocalToWorldMatrices);
            FastCopy(rbCentersOfMass, _waterTriJob.RigidbodyCOMs);
            FastCopy(rbLinearVelocities, _waterTriJob.RigidbodyLinearVels);
            FastCopy(rbAngularVelocities, _waterTriJob.RigidbodyAngularVels);
            ...
            _waterJobHandle = _waterTriJob.Schedule(n, 64);
        }
        ...
        private void Process()
        {
            ...
            FastCopy(_waterTriJob.States, _states);
            FastCopy(_waterTriJob.ResultForces, _forces);
            FastCopy(_waterTriJob.ResultPoints, _points);
            FastCopy(_waterTriJob.Normals, _normals);
            FastCopy(_waterTriJob.Areas, _areas);
            FastCopy(_waterTriJob.Velocities, _velocities);
            FastCopy(_waterTriJob.Distances, _distances);
            
            FastCopy(_waterTriJob.P00S, _P00S);
            FastCopy(_waterTriJob.P01S, _P01S);
            FastCopy(_waterTriJob.P02S, _P02S);
            FastCopy(_waterTriJob.P10S, _P10S);
            FastCopy(_waterTriJob.P11S, _P11S);
            FastCopy(_waterTriJob.P12S, _P12S);
        ...
        private static void FastCopy<T>(T[] source, NativeArray<T> destination) where T : struct
        {
            unsafe
            {
        ...
        private static void FastCopy<T>(NativeArray<T> source, T[] destination) where T : struct
        {
            unsafe
            {
                int byteLength = source.Length * UnsafeUtility.SizeOf(typeof(T));
